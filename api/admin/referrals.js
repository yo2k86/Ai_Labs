...
export default async function handler(req, res) {
    ...
    try {
        // PERBAIKAN: Menggunakan collectionGroup yang sangat cepat (Hanya 1x request ke database)
        // Ini mencegah terjadinya Error 500 (Timeout) di server Vercel
        const profilesSnapshot = await db.collectionGroup('profile').get();
        
        let usedCodesMap = {};
        
        // Membaca semua data profil pengguna secara bersamaan
        for (const profileDoc of profilesSnapshot.docs) {
            // Membongkar path dokumen untuk mendapatkan appId dan userId
        ...
        res.status(200).json({ success: true, codes: result });

    } catch (error) {
        console.error("Referrals Sync Error:", error);
        res.status(500).json({ error: "Gagal mensinkronkan status kode: " + error.message });
    }
}
